...
(function () {
  ...
  const Fonts = [
    { lbl: "شبنم", value: "'Shabnam', sans-serif" },
    { lbl: "Open Sans", value: "'Open Sans', sans-serif" },
    { lbl: "Oswald", value: "'Oswald', sans-serif" },
    { lbl: "Playfair Display", value: "'Playfair Display', sans-serif" },
    { lbl: "Cormorant Garamond", value: "'Cormorant Garamond', serif" },
    { lbl: "Lucida Console", value: "'Lucida Console', Monaco, monospace" },
    { lbl: "Dancing Script", value: "'Dancing Script', cursive" },
    { lbl: "Indie Flower", value: "'Indie Flower', cursive" },
    { lbl: "Amatic SC", value: "'Amatic SC', cursive" },
    { lbl: "Permanent Marker", value: "'Permanent Marker', cursive" },
  ];
  ...
  var selectionSettings = function () {
    ...
    ).innerHTML += `<div class="toolpanel" id="select-panel"><div class="content"><p class="title">تنظیمات لایه</p></div></div>`;

    const panelContent = document.querySelector(
      `${this.containerSelector} .toolpanel#select-panel .content`
    );

    const contentSelector = `${this.containerSelector} .toolpanel#select-panel .content`;
    ...
    $(contentSelector).append(`
        <div class="text-section">
          ...
          <div class="family">
            ...
            ${Fonts.map(
              (font) => `
                <option value="${font.value.trimStart()}">${font.lbl}</option> `
            )}
            </select>
            </div>
          </div>
          <div class="sizes">
            <div class="input-container"><label>اندازه قلم</label>
            ...
            <div class="input-container"><label>فاصله خطوط</label>
              <div class="custom-number-input">
              <button class="decrease">-</button>
            ...
            <div class="input-container"><label>فاصله حروف</label>
              <div class="custom-number-input">
              <button class="decrease">-</button>
          ...
          <div class="align">
            ...
            <select id="text-align">
              ...
              <option value="justify">هم راستا</option>
            </select>
            </div>
          </div>
          <div class="color">
    ...
    panelContent.querySelectorAll(".style button").forEach((btn) => {
    ...
    const setFontChangeHandler = ({ el, type }) => {
      panelContent
        .querySelector(el)
        .addEventListener("change", ({ target }) => {
          setActiveFontStyle(_self.activeSelection, type, target.value);
          _self.canvas.renderAll(), _self.canvas.fire("object:modified");
        });
    };

    setFontChangeHandler({ el: ".family #font-family", type: "fontFamily" });
    setFontChangeHandler({ el: ".align #text-align", type: "textAlign" });
    setFontChangeHandler({ el: ".color #color-picker", type: "fill" });

    panelContent
      .querySelector(`.sizes input`)
      .addEventListener("change", ({ target }) => {
    ...
    $(contentSelector).append(`
        <div class="border-section">
          <h4>حاشیه</h4>
          <div class="input-container"><label>وزن</label>
            <div class="custom-number-input">
            <button class="decrease">-</button>
          ...
          <div class="input-container"><label>استایل</label><select id="input-border-style">${BorderStyleList.map(
            (item) =>
              `<option value='${JSON.stringify(item.value)}'>${
                item.label
              }</option>`
          )}</select></div>
          <div class="input-container"><label>گوشه ها</label><select id="input-corner-type"><option value="miter" selected>صاف</option><option value="round">گرد</option></select></div>
          <div class="input-container"><label>رنگ</label><input id="color-picker" value="black"></div>
          <hr>
        </div>
      `);
    ...
    const setBorderChangeHandler = ({ el, handler }) => {
      panelContent
        .querySelector(el)
        .addEventListener("change", ({ target }) => {
          _self.canvas
            .getActiveObjects()
            .forEach((obj) => handler(obj, target.value));
          _self.canvas.renderAll(), _self.canvas.fire("object:modified");
        });
    };

    setBorderChangeHandler({
      el: ".border-section #input-border-width",
      handler: (obj, value) =>
        obj.set({
          strokeUniform: true,
    ...
    setBorderChangeHandler({
      el: ".border-section #input-border-style",
      handler: (obj, value) => {
    ...
    setBorderChangeHandler({
      el: ".border-section #input-corner-type",
      handler: (obj, value) => obj.set("strokeLineJoin", value),
    });

    // end border section

    // fill color section
    $(contentSelector).append(`<div class="color-settings"></div>`);

    createColorTabs({
      el: ".color-settings",
      container: ".toolpanel#select-panel .content",
      ...
      gradientHeight: () => _self.activeSelection.height,
      gradientWidth: () => _self.activeSelection.width,
    });
    ...
    panelContent.querySelectorAll(".alignment-section button").forEach((btn) =>
      btn.addEventListener("click", () => {
    ...
    $(contentSelector).append(`
        <div class="effect-section">
          <h4>افکت ها</h4>
          <div class="input-container"><label>شفافیت</label><input id="opacity" type="range" min="0" max="1" value="1" step="0.01"></div>
          <div class="input-container"><label>کدرشدن</label><input class="effect" id="blur" type="range" min="0" max="100" value="50"></div>
          <div class="input-container"><label>درخشندگی</label><input class="effect" id="brightness" type="range" min="0" max="100" value="50"></div>
          <div class="input-container"><label>خلوص رنگ</label><input class="effect" id="saturation" type="range" min="0" max="100" value="50"></div>
          <h5>گاما</h5>
          <div class="input-container"><label>قرمز</label><input class="effect" id="gamma.r" type="range" min="0" max="100" value="50"></div>
          <div class="input-container"><label>سبز</label><input class="effect" id="gamma.g" type="range" min="0" max="100" value="50"></div>
          <div class="input-container"><label>آبی</label><input class="effect" id="gamma.b" type="range" min="0" max="100" value="50"></div>
          <hr>
        </div>
      `);
    panelContent
      .querySelector(".effect-section #opacity")
      .addEventListener("change", ({ target }) => {
        let opacity = parseFloat(target.value);
        _self.activeSelection.set("opacity", opacity);
        _self.canvas.renderAll(), _self.canvas.fire("object:modified");
      });

    panelContent.querySelectorAll(".effect-section .effect").forEach((eff) => {
      eff.addEventListener("change", ({ target }) => {
        let effect = target.id;
        let value = parseFloat(target.value);
