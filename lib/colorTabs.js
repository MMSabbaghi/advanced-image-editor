function createColorTabs({
  ...
  const mainSelector = `${container} ${el}`;

  containerEl.querySelector(el).appendHTML(`
  ...
  const tabLabels = document.querySelectorAll(`${mainSelector} .tab-label`);
  const tabContents = containerEl.querySelectorAll(
    `${mainSelector} .tab-content`
  );
  ...
  $(`${mainSelector} #color-picker`).spectrum({
    flat: true,
    showPalette: false,
    showButtons: false,
    type: "color",
  ...
  const gp = new Grapick({
    el: `${mainSelector} #gradient-picker`,
  ...
  const updateGradientFill = () => {
    const orientationEl = document.querySelector(
      `${mainSelector} .gradient-orientation-container`
    );
