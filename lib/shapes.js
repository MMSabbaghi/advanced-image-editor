...
(function () {
  ...
  const shapes = function () {
    ...
    ).innerHTML += `<div class="toolpanel" id="shapes-panel"><div class="content"><p class="title">شکل ها</p></div></div>`;
    ...
    ShapeList.forEach(
      (svg) =>
        (panelContent.innerHTML += `<div onclick="renderSvg(this.innerHTML)" class="button">${svg}</div>`)
    );

    // onclick="renderSvg(this.innerHTML)"
